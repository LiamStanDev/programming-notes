...
### 編譯默認行為
##### 移動
* `return v2` : 當作函數返回值，表示該值除了被變量接收以外，不會再使用它了，故編譯器直接進行移動。
* `v1 = std::vector<int>(200)` : 注意這邊 v1 已經初始化了，但因為右半邊是一個右值(當場創建)，除了被 v1 接收以外不會再使用它了，故會直接移動而不會拷貝。
* `v1 = std::move(v2)` : 顯示調用移動

##### 拷貝
* `return std::as_const(v2)`
* `v1 = v2` : 默認拷貝

> 1. `std::move(t)` 等同於 `(T &&)t`, `std::const(t)` 等同於 `(const T&)t` : 只會進行類型轉換，移動與拷貝取決於後續的操作
> 2. `const T&` 為拷貝構造函數的參數類型，故等同於會調用拷貝構造
> 3. **只要不會再使用就會進行移動，只要還可能會使用就會進行拷貝**: 給定數值變量表示會再使用，而臨時數值不會再使用。
