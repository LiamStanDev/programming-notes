...
> 1. **有名字的爲左值**, 沒有名字的爲右值
> 2. `A&& arg` 爲一個右值引用**類型**，表示可以綁定右值, 但因爲 arg 有名字故本身爲**左值**，
...
* 萬能引用: 在模板下 `T&&` 可以接收左值與右值
* 引用摺疊: 在萬能引用下只有傳入值是右值參數本身才接收右值
* 完美轉發: 因爲參數接收到值之後一定爲左值（有名子）, 導致內部函數無法區分原始參數值爲左值還是右值
...
> 若不是萬能引用而是右值引用(e.g. `A&& arg`), 想要轉發右值還是得使用 `std::move(arg)`
...
* 概念: 預先建立好幾個物件(相同的), 讓調用方取得然後使用完自動回收但不釋放(只還原狀態)
* 目的: 避免頻繁分配/釋放:
* 場景: 連線池
