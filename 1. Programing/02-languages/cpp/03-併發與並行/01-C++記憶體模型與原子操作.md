...
* 起源：因爲編譯器與CPU指令優化，他們會優先考慮單執行續下若行爲一致的話，可以將前後順序進行重排，也就是你在程式碼中先改A再改B，可能會返過來，這問題在單執行續時沒有問題，但在多執行續下就要考慮了
```c++
# thread1: 進行寫入操作
data = 42; // 原本爲 52
ready = true;
...
# thread2:
if (ready) print(data) // 進行讀取操作時候 ready 可能已經被寫爲 true 但是 data 尚未被修改
```
...
**語意保證**:
- **Release**: 此操作前的所有的操作, 都不會被重排到該操作之後
	- Why: 將寫入想成發佈，你會希望你發佈對某個變數操作時，之前的操作都已經完成
- **Acquire**: 此操作後的所有的操作, 都不會被重排到該操作之前
	- Why: 將讀取當做消費方，**你會希望讀取值之後才進行後續的內容**
```cpp
std::atomic<bool> ready{false};
int data = 0;
