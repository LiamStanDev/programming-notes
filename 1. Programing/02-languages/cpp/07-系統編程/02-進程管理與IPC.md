...
| IPC 機制                                  | 延遲 (Latency)     | 頻寬 (Bandwidth) | 底層實現與延遲原因 (Internal Mechanics)                                                     | 應用場景與實例                                                             |
| :-------------------------------------- | :--------------- | :------------- | :--------------------------------------------------------------------------------- | :------------------------------------------------------------------ |
| **Pipe / FIFO**                         | 中 (~5 μs)        | 中              | 核心維護的**環形緩衝區 (Ring Buffer)**。數據需要**兩次複製** (用戶 $\to$ 核心 $\to$ 用戶)，涉及至少 **1 次系統呼叫**。 | **串流數據處理**、Unix Shell 管線 (如 `ls \| grep`、**單向**簡單進程通信。              |
| **Unix Socket**<br>(與 TCP/IP Socket 不同) | 中-高 (~10 μs)     | 中              | 核心內部的**緩衝區**，模擬 TCP/IP 協議棧邏輯。延遲略高於 Pipe，因需處理更多的 **協議結構**和 **多個系統呼叫**。              | **客戶端/伺服器 (C/S)** 模型、本地服務通信 (如 **Xorg** 伺服器與客戶端)、Docker Daemon API。 |
| **Message Queue**                       | 高 (~50 μs)       | 低              | 核心維護的**訊息佇列**。數據需要**兩次複製**。延遲高，因為核心需要處理訊息的 **結構化、優先級排序、核心鎖定**和**等待佇列管理**。          | **任務分發系統**、需要保證訊息順序或優先級的控制系統、**低頻率**的控制訊號傳遞。                        |
| **Shared Memory (Shm)**                 | **極低 (~100 ns)** | **極高**         | 核心分配實體內存頁面，**映射 (Map)** 到多個進程的**虛擬內存**。數據交換**無需系統呼叫、無需複製**。                        | **高速數據交換** (金融高頻交易 HFT)、多進程遊戲的**世界狀態**共享、巨大的數據緩衝區。                  |
| **Memory-Mapped File**                  | 極低 (~150 ns)     | 極高             | 使用 `mmap()` 將文件或匿名內存映射到進程地址空間。數據交換僅涉及**內存讀寫**，但首次訪問可能涉及**缺頁中斷**（磁碟 I/O）。           | **持久化共享數據**（跨應用程式重啟）、**零複製文件 I/O**、載入大型配置文件或資料庫文件。                  |
