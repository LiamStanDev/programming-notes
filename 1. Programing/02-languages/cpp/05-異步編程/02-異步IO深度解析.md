...
void async_io_example(int sockfd) {
	// 1. 初始化 io_uring 實例
    struct io_uring ring; // 聲明一個 io_uring 結構體變數，它代表了 io_uring 實例（I/O 佇列）
    
	// 初始化 io_uring 實例。 
	// 參數： 
	// - 256: 佇列的深度（queue depth），表示提交/完成佇列可以容納的最大請求數。 
	// - &ring: 指向 io_uring 結構體的指標。 
	// - 0: 標誌位（flags），0 表示使用預設行為。    
    ...
    // 從 Submission Queue (SQ) 獲取一個 Submission Queue Entry (SQE)。 
    // 這是提交一個新 I/O 請求的第一步。
    struct io_uring_sqe *sqe = io_uring_get_sqe(&ring);
    
    // 準備一個非同步的 recv (接收數據) 請求。 
    // 這個函數會填充上面獲取到的 sqe 結構體。 
    // 參數： 
    // - sqe: 要填充的 SQE 指標。 
    // - sockfd: 要讀取的 socket 文件描述符。 
    // - buffer: 接收數據的緩衝區。 
    // - sizeof(buffer): 緩衝區的大小（最大讀取字節數）。 
    // - 0: 標誌位（flags），對於 recv 通常為 0。
    io_uring_prep_recv(sqe, sockfd, buffer, sizeof(buffer), 0);
    
    // 為 SQE 設置一個用戶自定義的數據（user data）。 
    // 當請求完成時，這個數據會被返回到 Completion Queue Entry (CQE) 中。 
    // 這裡我們將 buffer 的地址作為用戶數據，以便在完成時知道哪個緩衝區被填充了（雖然本例中只有一個）。
    io_uring_sqe_set_data(sqe, buffer);
    
    // 將所有已填充的 SQE (在這個例子中只有一個) 提交給核心，開始非同步 I/O 操作。
    io_uring_submit(&ring);
    ...
    struct io_uring_cqe *cqe; // 聲明一個 Completion Queue Entry (CQE) 結構體指標。
    
    // 阻塞等待一個或多個完成的 I/O 請求。 
    // 當至少一個請求完成時，它會從 Completion Queue (CQ) 中彈出一個 CQE，並將其地址賦給 &cqe。
    io_uring_wait_cqe(&ring, &cqe);
    
    // 提取完成結果。cqe->res 包含了 I/O 操作的結果（例如，成功讀取的字節數或錯誤碼）。
    int result = cqe->res;
	if (result > 0) { 
		// 如果 result 大於 0，表示成功讀取了 result 個字節的數據。 
		std::cout << "Async read completed: " << result << " bytes\n"; 
		// 此時，socket 接收到的數據已經存儲在 buffer 變數中。 
	} else if (result == 0) { 
		// result 等於 0 通常表示對端關閉了連接（對於 socket recv）。 
		std::cout << "Async read completed: Peer closed the connection.\n"; 
	} else { 
		// result 小於 0 表示發生了錯誤。 
		std::cout << "Async read failed with error: " << -result << "\n"; 
	}
	
    // 4. 清理
    
    // 通知 io_uring 實例，這個 CQE 已經被處理（seen/consumed）。 
    // 這會將 CQE 從完成佇列中移除。
    io_uring_cqe_seen(&ring, cqe);
    
    // 釋放 io_uring 實例所使用的資源，包括內存映射。 
    // 這是程式結束前必須執行的步驟。
    io_uring_queue_exit(&ring);
}
```

**關鍵概念總結 (Quick Summary)**:
- `io_uring ring`: 核心結構，代表整個非同步 I/O 引擎。
- `io_uring_sqe` (Submission Queue Entry): 用戶空間填寫的結構，用來告訴核心要執行什麼 I/O 操作（例如 `recv`）。
- `io_uring_cqe` (Completion Queue Entry): 核心填寫的結構，用來通知用戶空間某個 I/O 操作已完成及其結果 (`cqe->res`)。
- `io_uring_submit`: 將已準備好的 SQE 推送到核心。
- `io_uring_wait_cqe`: 阻塞等待核心將完成的 CQE 推送回來。
...
**等待隊列 (Ready Queue)**:
* 存放調用了 `epoll_wait` 等待 I/O 事件發生的程序 (Process 或 Thread) 
* 若沒有任何 FD 準備好，核心會將當前正在執行 `epoll_wait` 的程序/執行緒加入到這個 `epoll` 結構的**等待隊列 (`wq`)** 中，並將該程序置於**休眠 (Sleep)** 狀態。
* 一旦 FD 被加入就緒鏈表，核心就會檢查 `eventpoll` 的**等待隊列 (`wq`)**。如果隊列中有休眠的程序，核心會立即將它們**喚醒 (Wake up)**。

```mermaid
graph TD
    subgraph "當沒有事件時"
        A["程序呼叫 epoll_wait(epfd)"] --> B{"就緒鏈表 rdllist 是否為空?"};
        B -- "是 (沒有事件)" --> C{"當前程序進入休眠狀態"};
        C --> D["程序加入 eventpoll::wq (等待隊列)"];
        D --> E["CPU 釋放給其他任務"];
    end

    style A fill:#DCEFFB,stroke:#007BFF
    style B fill:#FFFACD,stroke:#FFA500
    style C fill:#F0F0F0,stroke:#666
    style D fill:#F0F0F0,stroke:#666
    style E fill:#F0F0F0,stroke:#666

    subgraph "I/O 事件發生"
        F["FD 收到數據/寫緩衝區清空"] --> G["觸發 I/O 中斷"];
        G --> H["中斷處理程序執行回調"];
        H --> I["將 FD 加入就緒鏈表 rdllist"];
        I --> J{"檢查 eventpoll::wq (等待隊列)"};
        J -- "wq 非空 (有等待程序)" --> K["喚醒等待隊列中的所有程序"];
        K --> L["休眠程序被喚醒, 從 epoll_wait 返回"];
        J -- "wq 為空 (非阻塞或無等待)" --> L;
    end
    
    style F fill:#E6F0E6,stroke:#008000
    style G fill:#E6F0E6,stroke:#008000
    style H fill:#E6F0E6,stroke:#008000
    style I fill:#E6F0E6,stroke:#008000
    style J fill:#FFFACD,stroke:#FFA500
    style K fill:#E6F0E6,stroke:#008000
    style L fill:#DCEFFB,stroke:#007BFF

    E --> Z["等待 I/O 事件發生"];
    Z --> F;

    L --> M["程序處理 rdllist 中的就緒事件"];
```
...
#include <sys/epoll.h>
#include <sys/socket.h>
#include <netinet/in.h> // 網際網路協定族
#include <fcntl.h> // 檔案控制
#include <unistd.h> // POSIX 系統 API
#include <cstring>
#include <iostream>
