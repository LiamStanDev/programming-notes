# 金融 C++ 專題筆記 - 重組版

本目錄包含重新組織的 C++ 金融交易系統筆記,按主題分類,每篇筆記獨立自洽、完整可讀。

## ⭐ HFT 學習優先級

**所有筆記都已添加學習優先級標註**,幫助您高效安排學習順序:

- **⭐⭐⭐ 必看**: HFT 開發必須掌握的核心技術
- **⭐⭐ 建議**: 對 HFT 性能有重要影響的技術
- **⭐ 有空再看**: 有助於提升代碼質量的技術
- **無星**: 不太重要,可選學習

每份筆記的目錄都標註了各章節的優先級,建議按優先級順序學習。

## 已完成筆記

**01-C++核心特性.md**

- 智能指針 (unique_ptr, shared_ptr, weak_ptr)
- **6 個實戰練習**: LinkedList, BST, Graph, OrderBook, LRU Cache, 多態系統
- 移動語義、Lambda 表達式、模板編程
- C++17/20 新特性 (結構化綁定、Concepts、Ranges)
- RAII 與資源管理

**02-STL 容器與算法.md**

- 常用容器 (vector, deque, map, unordered_map, priority_queue)
- 常用算法 (sort, binary_search, partition)
- LeetCode 實戰案例與時間複雜度分析
- HFT 場景下的容器選擇指南

**03-並發與性能優化.md**

- 多線程基礎 (std::thread, thread_local)
- 同步原語 (mutex, condition_variable)
- 原子操作與內存序 (std::atomic, CAS)
- 無鎖編程 (無鎖棧、SPSC 隊列)
- 緩存優化 (False Sharing、預取)
- 線程池、性能測量
- **異步編程與協程** (std::async, C++20 Coroutines, 異步模式)

**04-編譯與優化.md**

- 編譯流程 (預處理、編譯、彙編、鏈接)
- 編譯器優化級別 (-O0 到 -O3/-Ofast)
- CPU 特定優化 (-march=native, SIMD)
- 鏈接時優化 (LTO)、性能引導優化 (PGO)
- 內聯優化、編譯器內建函數、CMake

**05-網路編程.md**

- TCP/UDP Socket API (客戶端、服務器、組播)
- Socket 選項配置 (TCP_NODELAY, keepalive, 緩衝區)
- epoll 基礎 (LT/ET 模式簡介)
- 零拷貝技術 (sendfile, mmap, splice)
- TCP 性能調優、應用層協議 (HTTP, WebSocket)
- 網路除錯工具 (tcpdump, netstat, strace)
- **注意**: 異步內容已移至 09-異步編程專題

**06-Linux 系統編程與調優.md**

- 進程管理 (fork, exec, IPC)
- 實時調度 (SCHED_FIFO, 優先級)
- CPU 親和性與隔離
- 內存管理 (mlock, Huge Pages, 內存池)
- 性能監控 (perf, /proc, strace)
- 系統調優 (中斷親和性、內核參數)

**07-金融交易系統專題.md**

- FIX 協議 (消息解析、構建)
- 市場數據處理 (Level 1/2, UDP 組播)
- 訂單簿實現 (基本版、高性能優化)
- 風險管理 (限額檢查、持倉管理)
- HFT 系統架構 (多線程設計)
- 完整系統實現 (SPSC 隊列、數據結構)

**08-常用庫與工具鏈.md**

- **序列化**: FlatBuffers ⭐⭐⭐⭐⭐, Protocol Buffers, JSON
- **網路**: Boost.Asio, ZeroMQ ⭐⭐⭐⭐⭐
- **日誌**: spdlog ⭐⭐⭐⭐⭐ (異步日誌)
- **測試與基準**: Google Test, Google Benchmark
- **數據結構**: Abseil (flat_hash_map)
- **時間**: Howard Hinnant's date
- **配置**: CLI11, yaml-cpp
- **HFT 專用**: QuickFIX

**09-異步編程專題.md** ⭐⭐⭐ **[新增]**

- **核心概念**: Future/Promise、事件循環、回調地獄解決方案
- **I/O 模型**: Reactor、Proactor、epoll 深度、io_uring
- **自定義異步庫**: EventLoop、Future、異步 TCP 完整實現
- **Boost.Asio**: C++17 回調 vs C++20 協程、SSL/TLS、連接池
- **io_uring**: 深度實踐、性能對比、批量優化、SQPOLL
- **異步資料庫**: libpq、libpqxx、PostgreSQL 連接池、事務處理
- **C++20 Coroutine**: 協程基礎、Asio 整合、異常處理、組合模式
- **最佳實踐**: 常見模式、錯誤處理、重試機制、性能測試
- **實戰項目**: 完整異步交易系統架構與實現

## 特色亮點

### 智能指針實戰練習 (01-C++核心特性)

根據您的要求,特別加強了智能指針的實踐案例:

1. **LinkedList** - unique_ptr 實現單向鏈表
2. **Binary Search Tree** - unique_ptr 實現 BST
3. **Graph** - shared_ptr/weak_ptr 避免循環引用
4. **HFT OrderBook** - 真實場景的訂單簿
5. **LRU Cache** - 複雜數據結構管理
6. **多態系統** - 智能指針與繼承結合

每個練習都包含完整可編譯代碼、詳細註釋和使用示例。

### 符合 AGENTS.md 規範

- 繁體中文 + 英文術語標註
- 無 emoji
- 每篇筆記獨立自洽
- 包含參考資料
- 使用 Mermaid 圖表

## 推薦學習路徑

### 第 1-3 天: 基礎

- 01-C++ 核心特性 (重點練習智能指針案例)
- 03-編譯與優化基礎

### 第 4-7 天: 進階

- 03-並發與性能優化
- 05-網路編程 (Socket 基礎)
- 06-Linux 系統編程與調優

### 第 8-12 天: 專家

- 09-異步編程專題 ⭐⭐⭐ (重點)
- 07-金融交易系統專題
- 08-常用庫與工具鏈

## 原始文件映射

| 新文件              | 原始來源                          | 備註                    |
| ------------------- | --------------------------------- | ----------------------- |
| 01-C++ 核心特性     | Day01, Day02                      | 移除冗餘 STL/Libs       |
| 02-STL 容器與算法   | **新增** (LeetCode 導向)          | 完整覆蓋 STL            |
| 03-並發與性能       | Day04, Day05                      | 移除異步內容            |
| 04-編譯與優化       | Day03                             |                         |
| 05-網路編程         | Day06, Day07 (部分)               | 移除異步內容            |
| 06-Linux 系統編程   | Day08                             | 包含 CPU 親和性         |
| 07-金融交易系統     | Day09, Day10                      |                         |
| 08-常用庫工具鏈     | **新增** (FlatBuffers, spdlog 等) |                         |
| 09-異步編程專題     | **新增** (Day06/07 異步部分整合)  | 完整異步編程知識體系    |

**總計**: **9 個主題筆記**，內容完整、職責清晰、無冗餘

## 使用指南

每個筆記文件都是獨立自洽的,可以單獨閱讀和學習。文件之間有適當的交叉引用,但不依賴特定的閱讀順序。

建議通過實際編寫代碼來學習,特別是智能指針的練習案例,這將幫助您深入理解 C++ 的內存管理和 RAII 原則。
