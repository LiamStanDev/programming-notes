...
| 枚舉值 | 描述 | 同步效果 | 開銷 |
|--------|------|----------|------|
| `memory_order_relaxed` | 鬆散序 | 無同步,僅保證原子性 | 最低 |
| `memory_order_acquire` | 獲取序 | 讀操作,禁止後續讀寫重排到此操作前 | 低 |
| `memory_order_release` | 釋放序 | 寫操作,禁止前面讀寫重排到此操作後 | 低 |
| `memory_order_acq_rel` | 獲取釋放序 | 同時具有 Acquire 和 Release 語義 | 中 |
| `memory_order_seq_cst` | 順序一致性 | 全局順序,所有執行緒看到相同的操作順序 | 高 |
| `memory_order_consume` | 消費序 | (不建議使用,標準中暫時不鼓勵) | - |
