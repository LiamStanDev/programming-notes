...
### 重要目錄說明
|**目錄**|**用途描述**|**核心機制**|**關鍵內容或應用**|**相關工具/服務**|
|---|---|---|---|---|
|**`/dev`**|**設備文件介面**：提供 I/O 接口與硬體或虛擬設備互動。|**設備節點** (字符/塊設備)。|`sda` (硬碟), `tty` (終端機), `null`, `random` (亂數源)。|`udev`, `mount`, `dd`|
|**`/proc`**|**程序與系統狀態**：提供運行時的動態程序、核心狀態和性能數據。|**Procfs** (虛擬檔案系統)，數據是動態生成的。|`/proc/[PID]/status` (程序狀態), `cpuinfo`, `meminfo`, `/proc/sys` (可調整的核心參數)。|`top`, `ps`, `free`, `sysctl`|
|**`/sys`**|**設備與核心拓撲**：提供結構化的核心設備樹和驅動程序配置介面。|**Sysfs** (虛擬檔案系統)，以層次結構組織設備屬性。|`/sys/class/net/eth0/` (網卡屬性), `/sys/devices/` (硬體拓撲), `/sys/block/` (塊設備資訊)。|`udev`, `lshw`, `lspci`|
|`/bin`|基本使用者命令：系統單用戶模式下也必須可用的基本執行檔。|實體執行檔|`ls`, `cp`, `mv`, `bash`, `dmesg`。|N/A|
|`/boot`|開機所需檔案：包含引導加載器和核心文件。|實體檔案系統|`vmlinuz` (Linux 核心), `initramfs` (初始 RAM 檔案系統), `grub` (引導加載器配置)。|`grub-install`, `update-initramfs`|
|`/etc`|**系統配置檔**：包含系統範圍的靜態配置資訊。|實體檔案系統|`passwd`, `fstab`, `nginx.conf`, `systemd` 配置。|N/A|
|`/home`|使用者家目錄：所有非 root 使用者的個人數據和配置。|實體檔案系統|`/home/user`, `.bashrc`, `.ssh/`。|N/A|
|`/lib`, `/lib64`|共享函式庫：核心模組和基本程式運行所需的基本共享函式庫。|實體檔案系統|`libc.so.6`, 核心模組 (被 `/lib/modules` 取代)。|`ldd`, `ldconfig`|
|`/mnt`, `/media`|臨時掛載點：用於臨時掛載外部或臨時設備。|實體檔案系統|USB 隨身碟、光碟機掛載點。|`mount`, `umount`|
|`/opt`|第三方軟體：用於安裝獨立的、大型的、非系統包管理器維護的應用程序。|實體檔案系統|專有軟體包、獨立編譯的應用程序。|N/A|
|`/root`|root 使用者家目錄：系統管理員的私人目錄。|實體檔案系統|N/A|N/A|
|`/run`|執行時資料：儲存系統啟動後生成的非永久性狀態數據。|**Tmpfs** (記憶體檔案系統，重啟清空)。|PID 檔案 (`httpd.pid`), 系統 socket。|`systemd`|
|`/srv`|服務資料：用於網站、FTP 等服務使用的數據。|實體檔案系統|網站根目錄 (`/srv/www`), Git 倉庫。|N/A|
|`/tmp`|暫存檔案：供程序創建臨時檔案的空間。|實體檔案系統或 **Tmpfs** (重啟或定期清空)。|N/A|N/A|
|`/usr`|使用者應用程式：系統中絕大多數的應用程序、函式庫和文件。|實體檔案系統|`/usr/bin` (應用程式), `/usr/lib` (函式庫), `/usr/share` (共享數據)。|N/A|
|`/var`|可變資料：包含系統運行時變動的數據，例如日誌、郵件和緩存。|實體檔案系統|`/var/log` (日誌), `/var/cache` (緩存), `/var/spool` (郵件佇列)。|`journalctl`, `logrotate`|
### VFS 虛擬檔案系統
